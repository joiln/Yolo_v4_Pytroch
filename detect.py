...
class VideoTracker(object):
    def __init__(self, cfg):
        ...
        self.carDirection = {}
        self.carPre = {}
        self.virtureLine = [[0, 0], [0, 0]]  # 进出虚线
        self.carInCnt = 0   #车进出数
        self.carOutCnt = 0
        self.inCar = set() #车进 集合
        self.outCar = set() #车出 集合
        self.trafficLine = []
        self.idx_frame = 0  # 视频画面第几帧
    ...
    def detectTrafficLine(self, img):  #识别人行横道
        Ni, Nj = (90, 1300)
    ...
    def run(self, path, sinImg, sin2, sinInfo, sinIllegal, config):
        ...
        while self.vdo.grab():
            ...
            cv2.line(image, (0, int(image.shape[0] / 2)), (int(image.shape[1]), int(image.shape[0] / 2)),
                     color=(10, 223, 224), thickness=5)
            im1 = ori_im.copy()
            im = im1

            bbox_xywh, cls_conf, cls_ids, ori_im = self.detector.detect_image(ori_im, self.trafficLine, self.outputDir,
                                                                              self.idx_frame, self.illegal, config)
                ...
                for i, bbox in enumerate(bbox_xyxy):
                    ...
                    if (self.carPre.get(id, 0) == 0):
                        ...
                        if self.carSpeed.get(id, 0) > self.carSpeedLimit:
                            ...
                            self.illegal[self.idx_frame].update({'overspeed': str(id) + " " + self.carLicense.get(id,
                                                                                                                  "") + " " + self.carLabel.get(
                                id, "")})
                            imgg.save(
                        ...
                        if self.carTurn.get(id, 0) == 'LEFT' and (
                                self.carFromRight.get(id, 0) == True or self.carFromForward.get(id,
                                                                                                0) == True) and self.carFromLeft.get(
                                id, 0) != True:

                            ori_im = pil_draw_box_2(ori_im, [x1, y1, x2, y2], label="未按导向行驶", font=font)
                            if isinstance(self.illegal.get(self.idx_frame, 0), int):
                                self.illegal[self.idx_frame] = {}
                            self.illegal[self.idx_frame].update({'turnwrong': str(id) + " " + self.carLicense.get(id,
                                                                                                                  "") + " " + self.carLabel.get(
                                id, "")})
                            if os.path.exists(self.outputDir + "illegal/"):
                                pass
                    ...
                    if self.carDirection.get(id, 0) == 'UP':
                        if len(config.laneline) > 0:
                            ...
                            if self.judge_line_illegal(id, tmp, config):
                                ...
                                if isinstance(self.illegal.get(self.idx_frame, 0), int):
                                    self.illegal[self.idx_frame] = {}
                                self.illegal[self.idx_frame].update({'touchline': str(id) + " " + self.carLicense.get(
                                    id, "") + " " + self.carLabel.get(id, "")})
                                if os.path.exists(self.outputDir + "illegal/"):
                                    pass
                                else:
                                    os.mkdir(self.outputDir + "illegal")
                    ...
                    self.carInfor[id] = self.carDirection.get(id, '') + "," + self.carLicense.get(id, '') + "," + str(
                        int(self.carSpeed.get(id, 0))) + "," + self.carLabel.get(id, '')

                # ori_im = draw_boxes(ori_im, bbox_xyxy, identities,self.carSpeed)
                font = ImageFont.truetype(font='model_data/simhei.ttf',
                                          size=np.floor(0.012 * np.shape(ori_im)[1]).astype('int32'))
                ori_im = pil_draw_box(ori_im, bbox_xyxy, identities, self.carSpeed, self.carLicense, self.carLabel,
                                      self.carDirection, font)
            ...
            ori_im = cv2.putText(ori_im, "carIn: %d carOut:%d" % (len(self.inCar), len(self.outCar)), (0, 150),
                                 cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 255, 0), 2)
            if len(self.trafficLine1) > 0 or len(self.trafficLine2) > 0:
                if len(self.trafficLine1) > 0:
                    plot_one_box(self.trafficLine1, ori_im, label="TrafficLine1", color=(205, 210, 104),
                                 line_thickness=3)
                if len(self.trafficLine2) > 0:
                    plot_one_box(self.trafficLine2, ori_im, label="TrafficLine2", color=(205, 210, 104),
                                 line_thickness=3)
    ...
    def classify_speed(self, id): # 速度等级
        carID = id
    ...
    def carrunred(self, id, x1, x2, y1, y2, ori_im):
        ...
        if (
                self.detector.trafficLightColor == 'red' or self.detector.forwardlightColor == 'red') and self.carDirection.get(
                id, 0) == 'UP' and self.carSpeed.get(id, 0) >= 10:
            ...
            self.illegal[self.idx_frame].update(
                {'carrunred': str(id) + " " + self.carLicense.get(id, "") + " " + self.carLabel.get(id, "")})
            imgg = imgTmp.copy()
            imgg = Image.fromarray(cv2.cvtColor(imgg, cv2.COLOR_BGR2RGB))
            imgg.save(self.outputDir + "illegal/carrunred/" + str(self.idx_frame) + "_" + self.carLicense.get(id,
                                                                                                              '') + ".jpg")
            font = ImageFont.truetype(font='model_data/simhei.ttf',
                                      size=np.floor(0.012 * np.shape(ori_im)[1]).astype('int32'))
    ...
    def save_csv_data(self):
        ...
        if self.idx_frame % 24 == 0:
            with open(self.outputDir + "csv/count.csv", 'a', newline='') as csvfile:
                ...
                writer.writerow([int(self.idx_frame / 24), int(self.detector.carCnt), int(self.detector.personCnt),
                                 int(self.detector.motoCnt)])
    ...
    def generate_chart(self):
        ...
        if os.path.exists(self.outputDir + "csv/flow.csv"):
            with open(self.outputDir + "csv/flow.csv", 'r') as f:           # with A as B 就和java中的try...catch一样可以捕获异常，先执行A，成功吧结果返回给B，反之utr-+
                reader = csv.reader(f)
                result = np.array(list(reader))
                for i in range(len(result[0])):
                    if i == 0:
                ...
                line2.render(self.outputDir + "chart/flow.html")            # render 作用是渲染图表到 HTML 文件
            ...
            with open(self.outputDir + "csv/speed.csv", 'r') as f:
                ...
                pie.add("速度区间",
                        ["[5km/h,10km/h)", "[10km/h,15km/h)", "[15km/h,20km/h)", "[20km/h,25km/h)", "[25km/h,30km/h)",
                         "[30km/h,40 km/h)", "[40km/h,50 km/h)", "[50km/h,+infinity)"], result[-1])
