...
class preDrawWidget(QWidget):           # 预处理框初始化
    def __init__(self, config, pg):
        super().__init__()
        self.pg = pg
        self.vlayout = QVBoxLayout(self)
        self.setWindowIcon(QIcon('./img/proc.svg'))
        self.setWindowTitle("视频预处理")

        self.label1 = QLabel("系统自动跟踪行人与汽车，并检测是否违规")
        self.label2 = QLabel("本软件会检测以下物体")

        self.forwardlineCheck = QCheckBox("前行车道线", self)  # 前行车道线
...
class progressWidget(QWidget):      # 处理详情框
    def __init__(self, path, screen, livepage, config):     # 对progressWidget 初始化
        super().__init__()
        self.screen = screen
        self.path = path
        ...
        self.vlayout = QVBoxLayout(self)  # 设置一个layout界面
        self.vlayout.addWidget(self.label)  # 给vlayout添加一个label
        self.vlayout.addWidget(self.progressbar)  # 给vlayout添加一个进度条

        self.vlayout.addWidget(self.endBtn)  # 给vlayout添加一个结束按钮
        self.setLayout(self.vlayout)
...
class SettingPage(QWidget):    # 设置页面
    ...
    def saveConfig(self):
        ...
        with open('settings.json', 'w') as f:
            setting_dict = {'yoloweight': self.config.YOLOWEIGHT, 'carspeedlimit': self.config.CARSPEEDLIMIT,
                            'trafficjamlimit': self.config.TRAFFICJAMLIMIT, 'savevideo': self.config.SAVEVIDEO,
                            'carclassify': self.config.CARCLASSIFY}

            json.dump(setting_dict, f)
        self.readSetting()

    def initUI(self):           # 初始化设置界面
...
class LivePage(QMainWindow):    #live 界面
...
class MyClass(QWidget):
    ...
    def on_btn_addVideo_clicked(self):  # 打开视频按钮
        ...
        self.browser.load(QUrl(QFileInfo('./output/video-022-1/chart/count.html').absoluteFilePath()))
    ...
    def changeView(self, x):
        if x == 0:
           ...
           self.browser.load(QUrl(QFileInfo('./output/video-022-1/chart/flow.html').absoluteFilePath()))
            ...
            self.browser.load(QUrl(QFileInfo('./output/video-022-1/chart/speed.html').absoluteFilePath()))
